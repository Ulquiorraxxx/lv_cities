CREATE TABLE cities(title VARCHAR(100),year INTEGER,population INTEGER,region VARCHAR(100));
...
SELECT title, year FROM cities WHERE year<1924;
...
SELECT COUNT(title) AS pilsētu_skaits FROM cities;
SELECT DISTINCT region FROM cities;
DELETE FROM cities WHERE title="Durbe";
SELECT region ,COUNT(population) ,SUM(population) FROM cities GROUP BY region;
